    ...
    )
{
    ...
    auto UdpSocket = CreateTcpUdpSocket(AF_INET, NULL, &LocalPort, TRUE);
    ...
    )
{
    ...
    auto UdpSocket = CreateTcpUdpSocket(Af, &If, &LocalPort, TRUE);
    auto GenericMp = MpOpenGeneric(If.GetIfIndex());
    wil::unique_handle ProgramHandle;
    ...
    )
{
    ...
    auto UdpSocket = CreateTcpUdpSocket(Af, &If, &LocalPort, FALSE);
    auto GenericMp = MpOpenGeneric(If.GetIfIndex());
    wil::unique_handle ProgramHandle;
    unique_malloc_ptr<UINT8> PortSet;
    ...
    const UCHAR GenericTcpPayload[] = "GenericRxMatchUdp";
    const UCHAR QuicLongHdrUdpPayload[40] = {
        0x80, // IsLongHeader
        0x01, 0x00, 0x00, 0x00, // Version
        0x08, // DestCidLength
    ...
    if (MatchType == XDP_MATCH_QUIC_FLOW_SRC_CID) {
        ...
        UdpPayload = GenericTcpPayload;
        UdpPayloadLength = sizeof(GenericTcpPayload);
    }
    ...
    )
{
    ...
    auto UdpSocket = CreateTcpUdpSocket(Af, &If, &XskPort, TRUE);
    auto Xsk =
        CreateAndBindSocket(
...
GenericTxToRxInject()
{
    ...
    auto UdpSocket = CreateTcpUdpSocket(AF_INET, &If, &LocalPort, TRUE);
    auto Xsk =
        CreateAndBindSocket(
